...
public:
    ...
    inline void    addTime(const unsigned int& it, const int& time);
    inline void    setTime(const int& time);

    inline void    start_clock(const unsigned int& it);
    inline void    end_clock(const unsigned int& it, const bool& elapsedTimeIsAccumulated = false);
...
template <class T>
inline void TimeStatistics<T>::addTime(const unsigned int& it, const int& inputTime)
{
    if (it < m_ElapsedTimes.size())
    {
        m_ElapsedTimes[it] += inputTime;
...
template <class T>
inline void TimeStatistics<T>::end_clock(const unsigned int& it, const bool& elapsedTimeIsAccumulated /*= false */)
{
    if (it < m_StartClocks.size())
    {
        ...
        int totalTime = 0;
        if (elapsedTimeIsAccumulated)
        {
            totalTime = time(it) + (int)(duration_cast<duration<double, T>>(endClock - startClock)).count();
        }
        else
        {
            totalTime = (int)(duration_cast<duration<double, T>>(endClock - startClock)).count();
        }
